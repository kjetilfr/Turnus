      ...
      <div className="p-3">
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="bg-gray-50 dark:bg-gray-700">
                <th className="px-2 py-1.5 text-left font-medium text-gray-900 dark:text-white">Week</th>
                <th className="px-2 py-1.5 text-center font-medium text-gray-900 dark:text-white">Shifts</th>
                <th className="px-2 py-1.5 text-center font-medium text-gray-900 dark:text-white">Hours</th>
                <th className="px-2 py-1.5 text-center font-medium text-gray-900 dark:text-white">Days Off</th>
                <th className="px-2 py-1.5 text-center font-medium text-gray-900 dark:text-white">Workload</th>
              </tr>
            </thead>
            <tbody>
              {shiftsPerWeek.map((week, weekIndex) => (
                <tr key={weekIndex} className={`border-b border-gray-100 dark:border-gray-700 ${weekIndex % 2 === 0 ? 'bg-white dark:bg-gray-800' : 'bg-gray-50/50 dark:bg-gray-750/50'}`}>
                  ...
                  <td className="px-2 py-1.5 text-center text-gray-600 dark:text-gray-300">
                    {week.shifts}
                  </td>
                  <td className="px-2 py-1.5 text-center text-gray-600 dark:text-gray-300">
                  ...
                  <td className="px-2 py-1.5 text-center">
                    <span className={`px-1.5 py-0.5 rounded text-xs font-medium ${
                      week.shifts >= 6 ? 'bg-red-100 text-red-800 dark:bg-red-900/20 dark:text-red-300' :
                      week.shifts >= 4 ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/20 dark:text-yellow-300' :
                      week.shifts >= 2 ? 'bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-300' :
                      'bg-blue-100 text-blue-800 dark:bg-blue-900/20 dark:text-blue-300'
                    }`}>
                      {week.shifts >= 6 ? 'Heavy' :
                       week.shifts >= 4 ? 'Normal' :
                       week.shifts >= 2 ? 'Light' : 'Minimal'}
                    </span>
                  </td>
                </tr>
              ))}
              ...
              {/* Summary Row */}
              <tr className="border-t-2 border-gray-300 dark:border-gray-600 bg-gray-100 dark:bg-gray-700 font-medium">
                ...
                <td className="px-2 py-1.5 text-center text-green-600 dark:text-green-400">
                  {totalPlanHours.toFixed(1)}h
                </td>
                <td className="px-2 py-1.5 text-center text-purple-600 dark:text-purple-400">
                  {totalDaysOff}
                </td>
                <td className="px-2 py-1.5 text-center text-orange-600 dark:text-orange-400">
                  {avgWeeklyHours.toFixed(1)}h avg
                </td>
              </tr>
            </tbody>
          </table>
        ...
        {plan.duration_weeks > 2 && (
          <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
            <div className="text-center bg-red-50 dark:bg-red-900/20 rounded p-1.5">
              <div className="text-sm font-semibold text-red-600 dark:text-red-400">
                {maxShiftsInWeek}
              </div>
              <div className="text-xs text-red-500 dark:text-red-400">Peak week</div>
            </div>
            <div className="text-center bg-green-50 dark:bg-green-900/20 rounded p-1.5">
              <div className="text-sm font-semibold text-green-600 dark:text-green-400">
                {minShiftsInWeek}
              </div>
              <div className="text-xs text-green-500 dark:text-green-400">Light week</div>
            </div>
          </div>
        )}
      </div>
